...
#define NODES 11
#define STRANGE_EVENTS_LIMIT 10
...
volatile int change_route = 0;
volatile int cancel = 0;
volatile int news_option = 0;
volatile int start_route = -1;
volatile int init_poz = 0;
...
void add_strange_event();
void add_normal_event();
void add_some_cars();
void send_incidents_on_near_street(int status_parcurs_total);
void send_aglomeration_event(int status_parcurs_total);

//pozitie
void initializare_pozitie(const char *primit, char* raspuns);
void *parcurgere_traseu(void *arg);
void setare_locatie_favorita(const char *primit, char* raspuns, char* username);
void set_destination(const char* primit, char* msg_de_trimis, char* username);
...
void update_cars_number(int nodeID, int increase);
void send_raspuns_stop(char* raspuns);
void send_neinitializat(char* raspuns);
...
char destinatie[] = "DST";
char locatie_fav[] = "FAV";
char stopped[] = "STP";

//Ce raspunsuri trimite serverul
char raspuns_stopped[] = "STP";
...
int check_near_streets(int status)
{
...
void *parcurgere_traseu(void *sock)
{
    ...
    while(cancel == 0)
    {
        ...
        {    
            ...
            if(check_near_streets(status_parcurs_total+1)==1)
            {
            ...
            {
                memset(msg_de_trimis, 0, sizeof(msg_de_trimis));
                sprintf(msg_de_trimis, "LOF: %s ati ajuns la destinatie", locatie_curenta);
                status_parcurs_per_strada = 0;
                status_parcurs_total = 1;
                msg_de_trimis[strlen(msg_de_trimis)] = '\0';
                send_function(msg_de_trimis);
            ...
            {   distance = get_distance(path[status_parcurs_total],path[status_parcurs_total+1]);
                if(status_parcurs_per_strada >= distance)
                {
                    ...
                    update_cars_number(path[status_parcurs_total], 1);
                    update_cars_number(path[status_parcurs_total-1], 0);
                    if(check_near_streets(status_parcurs_total+1)==1)
                    {
                        memset(msg_de_trimis, 0, sizeof(msg_de_trimis));
                        memset(next_street, 0, sizeof(next_street));
                    ...
                    if(strcmp(locatie_curenta, locatie_start) != 0)
                    {
                ...
                {   
                    printf("Distance:%f, viteza: %d", status_parcurs_per_strada + (float)viteza_int*0.06, viteza_int);
...
void strange_events_add_incident()
{
  char sql[MSGSIZE];
  char data[MSGSIZE];
  data[0] = 0;
  ...
  memset(data, 0, sizeof(data));
  sprintf(sql, "UPDATE Streets SET Incident = 1 where Strange_events > %d;", STRANGE_EVENTS_LIMIT);
  sql[strlen(sql)] = '\0';
  memset(data, 0, sizeof(data));
  rc = sqlite3_exec(db, sql, callback_without_name, data, &zErrMsg);
  fflush(stdout);
...
int  exists_home(char* username, char* home)
{
  char sql[MSGSIZE];
  char data[MSGSIZE];
  int res;
  ...
  sprintf(sql, "SELECT Home from Clienti where Username = '%s';",username);
  sql[strlen(sql)] = '\0';
...
int exists_work(char* username, char* work)
{
  char sql[MSGSIZE];
  char data[MSGSIZE];
  int res;
...
void set_destination(const char* primit, char* msg_de_trimis, char* username)
{
  ...
  else if(strstr(possible_dest, "Home") != NULL &&  exists_home(username, home) == 1)
  {
    memset(locatie_stop, 0, sizeof(locatie_stop));
    strcpy(locatie_stop, home);
    start_route = dijkstra(Graph,get_nodeID(locatie_start), get_nodeID(locatie_stop), path);
    printf("%s", locatie_stop);
    change_route = 1;
    memset(msg_de_trimis, 0, sizeof(0));
    sprintf(msg_de_trimis,"DSE:%s ", locatie_stop);
    msg_de_trimis[strlen(msg_de_trimis)] = '\0';
  ...
  else if(strstr(possible_dest, "Work") != NULL && exists_work(username, work) == 1)
  {
    memset(locatie_stop, 0, sizeof(locatie_stop));
    strcpy(locatie_stop, work);
    start_route = dijkstra(Graph,get_nodeID(locatie_start), get_nodeID(locatie_stop), path);
    change_route = 1;
    memset(msg_de_trimis, 0, sizeof(0));
    sprintf(msg_de_trimis,"DSE:%s ", locatie_stop);
...
void send_raspuns_stop(char* msg_de_trimis)
{
  memset(msg_de_trimis, 0, sizeof(0));
  sprintf(msg_de_trimis,"STP:Trebuie sa fiti in miscare pentru a putea obtine informatii legate de trafic");
  msg_de_trimis[strlen(msg_de_trimis)] = '\0';
  send_function(msg_de_trimis);
}


void send_neinitializat(char* msg_de_trimis)
{
  memset(msg_de_trimis, 0, sizeof(0));
  sprintf(msg_de_trimis,"STP:Trebuie sa fiti in miscare pentru a putea obtine informatii legate de trafic");
  msg_de_trimis[strlen(msg_de_trimis)] = '\0';
...
int pregatire_raspuns(const char *primit, char *msg_de_trimis, int logat, char* username)
{
  ...
  else if(strstr(primit, stopped) != NULL)
  {
    logat = logat;
    init_poz = 0;
    send_raspuns_stop(msg_de_trimis);
  }
  ...
  else if(strstr(primit, locatie_fav) != NULL && logat == 1)
  {
    logat=logat;
    setare_locatie_favorita(primit, msg_de_trimis, username);
  }
  else if(strstr(primit, destinatie) != NULL && logat == 1)
  {
    logat = logat;
    if(init_poz == 0)
        send_neinitializat(msg_de_trimis);
    else
        set_destination(primit, msg_de_trimis, username);
  }
  else
  {
    logat = logat;
...
void setare_locatie_favorita(const char* primit, char* msg_de_trimis, char* username)
{
  char street[MSGSIZE];
  ...
  get_lines(primit, tip_locatie, street);


  memset(msg_de_trimis, 0, sizeof(msg_de_trimis));
  if(check_street_exists(street) && (strcmp(tip_locatie, "Work")==0 || strcmp(tip_locatie,"Home") == 0))
  {
    char sql[MSGSIZE];
    char data[MSGSIZE];
    data[0] = 0;
    ...
    memset(data, 0 ,sizeof(data));
    memset(sql, 0, sizeof(data));
    sprintf(sql, "UPDATE Clienti SET %s = '%s' WHERE Username = '%s';",tip_locatie, street, username);
    ...
    sprintf(msg_de_trimis, "FAV:Strada %s a fost setata drept %s", street, tip_locatie);
    msg_de_trimis[strlen(msg_de_trimis)] = '\0';
    send_function(msg_de_trimis);
  }
  else if(check_street_exists(street) == 0)
  {
    sprintf(msg_de_trimis, "FAV:Strada %s nu exista", street);
    msg_de_trimis[strlen(msg_de_trimis)] = '\0';
    send_function(msg_de_trimis);
  }
  else
  {
    sprintf(msg_de_trimis, "FAV:Tipul de locatie %s nu este valabil", tip_locatie);
    msg_de_trimis[strlen(msg_de_trimis)] = '\0';
    send_function(msg_de_trimis);
  }
...
void initializare_pozitie(const char *primit, char* msg_de_trimis)
{
  ...
  update_cars_number(get_nodeID(locatie_start), 1);
...
void speed_advertisment(const char *primit, char* msg_de_trimis)
{
  ...
  limita_viteza = check_speed_limit(viteza);
  
  if(limita_viteza == 1)
  {
    sprintf(msg_de_trimis, "%s:Ati depasit limita de viteza impusa pe sectorul de drum: %s",update_speed,locatie_curenta);
    msg_de_trimis[strlen(msg_de_trimis)] = '\0';
  ...
  if(viteza_int < limita_viteza-20)
  {
    add_strange_event();
  }
  else
...
void add_strange_event()
{
  ...
  char *zErrMsg = 0;
  int rc;
  if(strcmp(locatie_curenta, temp) != 0)
  {
    memset(temp, 0, sizeof(temp));
    strcpy(temp, locatie_curenta);
    sprintf(sql, "UPDATE Streets SET Strange_events = Strange_events +1 where StreetName = '%s';", locatie_curenta);
    sql[strlen(sql)] = '\0';
  }
  memset(data, 0, sizeof(data));
  rc = sqlite3_exec(db, sql, callback_without_name, data, &zErrMsg);
...
int functie_sign_in(const char *primit, char *msg_de_trimis, int logat, char *username)
{
  ...
  else
  {
    ...
    if(logat == 1)
    {
      ...
      char utilizatori[MSGSIZE];
      memset(utilizatori, 0, sizeof(utilizatori));
      sprintf(utilizatori, "UTL:%d utilizatori in apropiere", uid-11);
      utilizatori[strlen(utilizatori)] = '\0';
      send_function(utilizatori);
    }
  }
...
void update_speedlimit_incident(const char *primit)
{
  ...
  if(strcmp(type,"Accident") == 0 || strcmp(type,"Aglomeratie") == 0)
  {
...
void add_some_cars()
{
  ...
  {
    srand(time(NULL));
    sprintf(sql, "UPDATE Paths SET Cars = Cars+%d;", rand()%(NORMAL_TRAFIC/2));
    sql[strlen(sql)] = '\0';
    memset(data, 0, sizeof(data));
    rc = sqlite3_exec(db, sql, callback_without_name, data, &zErrMsg);
    fflush(stdout);
